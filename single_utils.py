...
def _get_temporal_param_str(args):
    """
    Return the trigger-specific parameter string for temporal triggers.
    Used in both path_name and save_experiments for consistency.
    """
    if args.type == 'latency':
        return f'df{args.delay_frames}'
    elif args.type == 'rate':
        return f'sf{args.scale_factor}'
    elif args.type == 'jitter_fixed':
        return f'ns{args.n_shift}'
    else:
        return None
...
def path_name(args):
    """
    Generate the path name based on the experiment arguments. Use a function for
    ...
    elif args.type in ('latency', 'rate', 'jitter_fixed'):
        temporal_str = _get_temporal_param_str(args)
        path = f'{args.dataset}_{args.type}_{args.epsilon}_{args.trigger_size}_{temporal_str}_{args.seed}'
    else:
...
def save_experiments(args, train_acc, train_loss, test_acc_clean, test_loss_clean, test_acc_backdoor,
    ...
    header = ['dataset', 'least', 'most_polarity', 'seed', 'epsilon', 'pos',
              ...
              'delay_frames', 'scale_factor', 'n_shift',
    ...
    with open(path, 'a') as f:
        ...
        writer.writerow([args.dataset, args.least, args.most_polarity, args.seed, args.epsilon, args.pos,
                         ...
                         args.delay_frames, args.scale_factor, args.n_shift,
